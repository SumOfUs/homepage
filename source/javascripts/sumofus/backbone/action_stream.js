const ActionStream = Backbone.View.extend({
  el: '.action-stream',
  ACTION_SOURCE: 'https://actions-api.eko.org',
  LINK_ROOT: 'https://actions.eko.org',
  MAX_PER_PERIOD: 1,
  ...
  updateTimestamps() {
    this.$('.stream-action__timestamp').each((ii, el) => {
      ...
      $el.text(
        I18n.t('homepage.action_stream.seconds_ago', { seconds: seconds })
      );
    });
  },
  ...
  renderActions() {
    ...
    for (var ii = 0; ii < countToAdd; ii++) {
      ...
      totalWidth += this.actionsContainer
        .children()
        .first()
        .outerWidth();
    }
    ...
    window.setTimeout(() => {
      this.actionsContainer.css(
        'transition',
        `left ${this.ANIMATION_DURATION}s ease-in-out`
      );
  ...
  template(action_type, name, seconds_ago, url) {
    if (action_type !== 'petition' && action_type !== 'donation') {
      console.error(
        `Invalid action_type '${action_type}'. Expected 'petition' or 'donation'.`
      );
      return;
    }
    ...
    return `<a class="action-stream__template stream-action" href="${url}">
              ...
              <div class="stream-action__text">
                ...
                <div class="stream-action__timestamp" data-seconds="${seconds_ago}">
                  ${I18n.t('homepage.action_stream.seconds_ago', {
                    seconds: seconds_ago,
                  })}
                </div>
              </div>
